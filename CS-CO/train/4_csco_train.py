...
def train_epoch(train_loader, model, model_type, loss_fn, optimizer, device,
                epoch, use_momentum):
    model.train()
    losses = []
...
if __name__ == '__main__':
    ...
    mpp = train_root.split('/')[-2]
    #print(mpp)    

    tot_list = data_list(train_root, k=None)
    train_list = tot_list[:training_size]
    eval_list = tot_list[training_size:]
    ...
    train_dataset = get_training_set(train_root, train_list, model_type=model_type)
    eval_dataset = get_validating_set(valid_root, eval_list, model_type=model_type) 
    ...
    early_stopping = EarlyStopping(patience=15, verbose=True, path=checkpoint_path+'{}_{}_{}_{}_{}_{}_{}_{}_{}.pth'.format(method_name,model_type, mpp, optim_name,training_size,batch_size,lr,weight_decay,moving_average_decay))
    
    
    for epoch in range(epochs):
        print('epoch {}/{}'.format(epoch+1, epochs))
        train_epoch(train_loader, model, model_type, loss_fn, optimizer,
                    device, epoch+1, use_momentum)
